...
jQuery.extend(Mgly.CodeMirrorDiffView.prototype, {
    ...
    scrollToDiff: function(direction) {
        if (this.changes === undefined || !this.changes.length) return;
        if (direction == 'next') {
    ...
    currentDiff: function() {
        if (this._current_diff !== undefined) {
            return this._current_diff + 1;
        }
        return 0;
    },
    scrollToLastDiff: function() {
        if (this.changes === undefined || !this.changes.length) return;
        this._current_diff = this.changes.length - 1;
        this._scroll_to_change(this.changes[this._current_diff]);
        this._changed(this.id + '-lhs', this.id + '-rhs');
    },
    scrollToFirstDiff: function() {
        if (this.changes === undefined || !this.changes.length) return;
    ...
    _markup_changes: function (editor_name1, editor_name2, changes) {
        ...
        led.operation(function() {
            for (var i = 0; i < changes.length; ++i) {
                ...
                led.addLineClass(llf, 'background', 'mergely-diff-start');
                led.addLineClass(llt, 'background', 'mergely-diff-end');
        ...
        red.operation(function() {
            for (var i = 0; i < changes.length; ++i) {
                ...
                red.addLineClass(rlf, 'background', 'mergely-diff-start');
                red.addLineClass(rlt, 'background', 'mergely-diff-end');
    ...
    _draw_diff: function(editor_name1, editor_name2, changes) {
        ...
        for (var i = 0; i < changes.length; ++i) {
            ...
            var fgcolor = this.settings.fgcolor[change['op']]
            if (this._current_diff == i) {
                var currentlySelectedFgColor = this.settings.fgcolor['c' + change['op']];
                if (currentlySelectedFgColor) {
                    fgcolor = currentlySelectedFgColor;
                }
            }


            ctx_lhs.beginPath();
            ctx_lhs.fillStyle = fgcolor;
